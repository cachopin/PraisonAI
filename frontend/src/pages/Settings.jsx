...
export default function Settings({ agents, onRefresh }) {
  const [company, setCompany] = useState({ name: 'My Company', industry: 'General', timezone: 'UTC' })
  const [defaults, setDefaults] = useState({ model: 'gpt-4o-mini', autoFire: false, autoFireThreshold: 3.0, arenaFreq: 'weekly' })
  const [notifications, setNotifications] = useState({ slack: '', email: '' })
    ...
    <div className="h-full overflow-y-auto p-6" style={{ background: '#060610' }}>
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm mt-1" style={{ color: '#5a5a7a' }}>Tune your entire silicon workforce from here.</p>
        </div>
        ...
        <Section title="COMPANY PROFILE">
          <div className="space-y-3">
            <Field label="COMPANY NAME">
              <input value={company.name} onChange={e => setCompany(c => ({ ...c, name: e.target.value }))} className="w-full px-3 py-2.5 rounded-lg text-sm" style={{ background: '#060610', border: '1px solid #1a1a30', color: '#d0d0e8' }} />
            </Field>
            <div className="grid grid-cols-2 gap-3">
              <Field label="INDUSTRY">
                <select value={company.industry} onChange={e => setCompany(c => ({ ...c, industry: e.target.value }))} className="w-full px-3 py-2.5 rounded-lg text-sm" style={{ background: '#060610', border: '1px solid #1a1a30', color: '#d0d0e8' }}>
                  {['General', 'Technology', 'Retail', 'Healthcare', 'Legal', 'Finance', 'Creative', 'Manufacturing', 'Consulting'].map(i => <option key={i}>{i}</option>)}
                </select>
              </Field>
              <Field label="TIMEZONE">
                <select value={company.timezone} onChange={e => setCompany(c => ({ ...c, timezone: e.target.value }))} className="w-full px-3 py-2.5 rounded-lg text-sm" style={{ background: '#060610', border: '1px solid #1a1a30', color: '#d0d0e8' }}>
        ...
        <Section title="AGENT DEFAULTS">
          <div className="space-y-3">
            <Field label="DEFAULT MODEL">
              <select value={defaults.model} onChange={e => setDefaults(d => ({ ...d, model: e.target.value }))} className="w-full px-3 py-2.5 rounded-lg text-sm" style={{ background: '#060610', border: '1px solid #1a1a30', color: '#d0d0e8' }}>
                {DEFAULT_MODELS.map(m => <option key={m}>{m}</option>)}
              </select>
            </Field>
            <Field label="ARENA FREQUENCY">
              <select value={defaults.arenaFreq} onChange={e => setDefaults(d => ({ ...d, arenaFreq: e.target.value }))} className="w-full px-3 py-2.5 rounded-lg text-sm" style={{ background: '#060610', border: '1px solid #1a1a30', color: '#d0d0e8' }}>
        ...
        <div className="flex justify-end mb-6">
          <button onClick={handleSave} className="px-6 py-2.5 rounded-xl text-sm font-mono font-bold transition-all" style={{ background: saved ? '#1a2a10' : '#a8ff3e', color: saved ? '#a8ff3e' : '#060610', border: saved ? '1px solid rgba(168,255,62,0.3)' : 'none' }}>
        ...
        <div className="rounded-xl p-5" style={{ background: 'rgba(255,61,61,0.04)', border: '1px solid rgba(255,61,61,0.2)' }}>
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs" style={{ color: '#5a5a7a' }}>Permanently delete all agents and their conversation history. Cannot be undone.</p>
            </div>
            <button onClick={handlePurge} className="px-4 py-2 rounded-lg text-xs font-mono shrink-0 ml-4 transition-colors" style={{ border: '1px solid rgba(255,61,61,0.4)', color: '#ff3d3d' }}>
              PURGE ALL
            </button>
          </div>
          <div className="mt-4 pt-4 border-t flex items-start justify-between" style={{ borderColor: 'rgba(255,61,61,0.1)' }}>
            <div>
              <p className="text-sm font-medium mb-0.5" style={{ color: '#f0f0ff' }}>Fleet size</p>
